...
const elevenlabsModel = elevenLabs.experimental_realtime('your-agent-id');
```

<Note>
  ElevenLabs agents must have tools registered on the agent. Passing
  `sessionConfig.tools` to `elevenLabs.experimental_realtime.getToken()`
  currently does not register tools dynamically.
</Note>
